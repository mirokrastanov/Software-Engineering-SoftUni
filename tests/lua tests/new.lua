...
ns.soulboundAtlas = isClassic and "AzeriteReady" or
"Soulbind-32x32"                                                                                                        -- UF-SoulShard-Icon-2x
...
local function PrepareItemButton(button)
    ...
    button.simpleilvlmissing:SetFont([[Fonts\ARIALN.TTF]], 13, "THICKOUTLINE")
...
local function AddMissingToButton(button, itemLink)
    ...
    button.simpleilvlmissing:SetFormattedText("%s%s", missingGems and ns.gemString or "",
        missingEnchants and ns.enchantString or "")
...
ns:RegisterAddonHook("Blizzard_VoidStorageUI", function()
    ...
    hooksecurefunc("VoidStorage_ItemsUpdate", function(doStorage, doContents)
        ...
        for i = 1, VOID_STORAGE_MAX do
            local itemID, textureName, locked, recentDeposit, isFiltered, quality = GetVoidItemInfo(
                VoidStorageFrame.page, i)
...
do
    ...
    function ns.ItemHasEmptySlots(itemLink)
        ...
        local gems = (gem1 ~= "" and 1 or 0) + (gem2 ~= "" and 1 or 0) + (gem3 ~= "" and 1 or 0) +
            (gem4 ~= "" and 1 or 0)
        return slots > gems
    end
