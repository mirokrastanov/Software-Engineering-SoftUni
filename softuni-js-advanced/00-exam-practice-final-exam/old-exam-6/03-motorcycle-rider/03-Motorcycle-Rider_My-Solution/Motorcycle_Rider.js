const motorcycleRider = {
  ...
  otherSpendings(equipment, consumables, discount) {
    if (!Array.isArray(equipment) || !Array.isArray(consumables) || typeof discount !== "boolean") throw new Error("Invalid Information!");
    let totalPrice = 0;
    equipment.forEach((element) => {
      if (element === "helmet") totalPrice += 200;
      else if (element === "jacked") totalPrice += 300;
    });
    consumables.forEach((element) => {
      if (element === "engine oil") totalPrice += 70;
      else if (element === "oil filter") totalPrice += 30;
    });
    if (discount) {
...
let text1 = motorcycleRider.otherSpendings(['helmet'], ['oil filter'], false);
console.log(text1);