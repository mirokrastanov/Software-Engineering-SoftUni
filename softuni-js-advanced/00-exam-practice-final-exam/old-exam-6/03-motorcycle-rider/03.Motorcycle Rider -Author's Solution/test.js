...
describe('Motorcycle Rider', () => {
    it('licenseRestriction', () => {
        ...
        expect( motorcycleRider.licenseRestriction('A2') ).to.equal('Motorcycles whit maximum power of 35KW. and the minimum age is 18.');
    ...
    it('otherSpendings', () => {
        ...
        expect(() => { motorcycleRider.otherSpendings(['helmet','jacked'],'oil filter', true) }).to.throw('Invalid Information!');
        expect(motorcycleRider.otherSpendings(['helmet','jacked'],[], true)).to.equal('You spend $450.00 for equipment and consumables whit 10% discount!'); 
        expect(motorcycleRider.otherSpendings(['helmet','jacked'],[], false)).to.equal('You spend $500.00 for equipment and consumables!'); 
        expect(motorcycleRider.otherSpendings(['helmet'],['engine oil'], true)).to.equal('You spend $243.00 for equipment and consumables whit 10% discount!'); 
        ...
        expect(motorcycleRider.otherSpendings([],['engine oil','oil filter'], true)).to.equal('You spend $90.00 for equipment and consumables whit 10% discount!'); 
